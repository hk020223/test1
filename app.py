...
3. **Step 3: 불연속 시간대(Split Times) 철저 준수**:
   - 강의 시간이 "월3, 수4"처럼 요일이나 교시가 떨어져 있는 경우, **해당되는 모든 셀(월3 셀, 수4 셀)에 각각 과목명과 교수명을 똑같이 기재**하세요.
   - 절대로 "월3" 하나만 채우거나, "월3,4"로 임의로 합치지 마세요.
   - PDF에 적힌 시간 패턴(예: `월3/수4`, `월(3),수(4)`)을 정확히 분리해서 해석하세요.
