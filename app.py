...
[엄격한 제약사항 - 위반 시 오답 처리]
1. **학년 매칭 필터링 (Grade Matching - 매우 중요)**:
   - PDF 문서의 표에서 **'학년(Year)' 또는 '이수학년' 열을 반드시 확인**하세요.
   - 사용자가 선택한 학년과 **일치하는 과목만** 시간표에 넣으세요.
   - **상위 학년 과목 절대 금지**: 예를 들어 1학년 시간표에 2, 3, 4학년 대상 과목(예: 공학수학, 회로이론 등)이 포함되면 안 됩니다.
   - 만약 '필수'라고 적혀 있어도, 대상 학년이 일치하지 않으면 과감히 제외하세요.

2. **사실 기반 생성 (Hallucination 방지)**:
...
3. **출력 형식 (세로형 HTML Table)**:
   ...
   - **스타일 규칙**:
     ...
     - 셀 내용: `<b>과목명</b><br><small>교수명 (대상학년)</small>`  <-- 학년 표기 필수!

4. **출력 순서 고정**:
   - **1순위:** HTML 시간표 표 (어떤 설명 텍스트도 없이 오직 `<table>...</table>` 코드만 가장 먼저 출력)
   - **2순위:** "### ✅ 필수 과목 검증" 섹션 (이유 설명, 예: 대학물리1 (1학년 필수))
   - **3순위:** "### ⚠️ 배치 실패 목록" 섹션
   - 이 순서를 절대 바꾸지 마라.
"""
...
def generate_timetable_ai(major, grade, semester, target_credits, blocked_times_desc, requirements):
    ...
    def _execute():
        ...
        [필수 지시사항 추가]
        - **필수 커리큘럼 준수**: 제공된 문서(요람 등)를 분석하여 '{major} {grade} {semester}'에 꼭 들어야 하는 필수 과목(전필, 기초교양 등)을 파악하세요.
